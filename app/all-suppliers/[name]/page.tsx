    ...
    toast((t) => (
      <div>
        ...
        <p className="text-sm text-gray-600 mb-4">
          This will delete the statement and all associated purchases, stock, payments, and ledger entries. This action cannot be undone.
        </p>
          ...
          <button
            onClick={async () => {
              ...
              try {
                ...
                const { data: purchases } = await supabase.from('cloth_purchases').select('id').eq('supplier_id', partyId);
                const purchaseIds = purchases?.map(p => p.id) || [];
                
                if (purchaseIds.length > 0) {
                  // Check if any cloth has been issued
                  const { data: linkedIssues, error: issuesError } = await supabase
                    .from('cloth_issues')
                    .select('id')
                    .in('cloth_purchase_id', purchaseIds)
                    .limit(1);

                  if (issuesError) throw issuesError;

                  if (linkedIssues && linkedIssues.length > 0) {
                    throw new Error('Cannot delete: Cloth from one or more purchases in this statement has already been issued. Please delete those issues first.');
                  }
                  
                  // Delete cloth stock
                  const { error: stockErr } = await supabase.from('cloth_stock').delete().in('purchase_id', purchaseIds);
                  if (stockErr) throw stockErr;
                }

                // 3. Delete all purchases related to this supplier statement
                const { error: err3 } = await supabase.from('cloth_purchases').delete().eq('supplier_id', partyId);
                if (err3) throw err3;

                // 4. Delete direct garment production records related to this supplier statement
                const { error: err4 } = await supabase.from('production_records').delete().eq('factory_id', partyId);
                if (err4) throw err4;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {sortedStatements.map((stmt, index) => (
            <Link 
              ...
              key={stmt.id}
              className="relative group bg-white rounded-xl shadow-sm border p-5 hover:shadow-md transition-all hover:border-orange-300 flex flex-col hover:z-[9999]"
            >
              <div
                role="button"
