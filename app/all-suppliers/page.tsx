  ...
  const fetchSuppliers = async () => {
    ...
    if (data) {
      const grouped = data.reduce((acc, curr) => {
        ...
        return acc
      }, {} as Record<string, SupplierParty>)
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-500">Manage your supplier statements and balances</p>
            </div>
          </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm border p-8 text-center">
          ...
          <p className="text-gray-500 mb-4">Start by adding a purchase to create a supplier statement.</p>
          <Link 
            href="/purchases/add"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredAndSortedSuppliers.map(supplier => (
            <Link 
              href={`/all-suppliers/${encodeURIComponent(supplier.name)}`} 
              key={supplier.id}
              ...
              <div className="flex-1">
                <div className="bg-gray-50 rounded-lg p-3">
                  ...
                  <p className={`text-xl font-bold ${
                    Number(supplier.current_balance) > 0 ? 'text-red-600' : 
                    Number(supplier.current_balance) < 0 ? 'text-green-600' : 
                    'text-gray-900'
                  }`}>
                    ₹{Math.abs(Number(supplier.current_balance)).toLocaleString('en-IN')}
                  </p>
                </div>
              </div>
