    ...
    <div className="max-w-7xl mx-auto px-4 py-6 pb-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div 
          ...
          className={`rounded-xl p-6 border flex items-center justify-between cursor-pointer transition-all duration-200 ${
        ...
        >
          <div>
            <p className="text-sm text-green-700 font-semibold uppercase tracking-wider mb-1">To Collect (They Owe Me)</p>
            <p className="text-3xl font-bold text-green-700">₹{totalReceivables.toLocaleString('en-IN')}</p>
          </div>
        ...
        >
          <div>
            <p className="text-sm text-red-700 font-semibold uppercase tracking-wider mb-1">To Pay (I Owe Them)</p>
            <p className="text-3xl font-bold text-red-700">₹{totalPayables.toLocaleString('en-IN')}</p>
          </div>
      ...
      {balanceFilter !== 'all' && (
        <div className="bg-white rounded-xl border mb-6 overflow-hidden">
          <div className="p-4 border-b bg-gray-50">
            <h3 className="font-bold text-gray-800">Outstanding Balances Breakdown</h3>
          </div>
            ...
            <div className="p-4">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2 text-orange-600">
                  ...
                  <h4 className="font-bold uppercase text-sm">Customers</h4>
                </div>
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto pr-2">
                {breakdownParties.filter(p => p.party_type === 'customer').length === 0 && (
                  <p className="text-sm text-gray-400 italic py-2">No matching balances</p>
                )}
                {breakdownParties.filter(p => p.party_type === 'customer').map(p => (
                  <div key={p.id} className="flex justify-between items-center py-1 border-b border-gray-50 last:border-0">
                    <Link href={`/statements/${p.id}`} className="text-sm text-gray-700 hover:text-blue-600 hover:underline truncate pr-2">
                      {p.name}
                    </Link>
                    <span className={`text-sm font-medium whitespace-nowrap ${Number(p.current_balance) > 0 ? 'text-green-600' : 'text-red-600'}`}>
            ...
            <div className="p-4">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2 text-purple-600">
                  ...
                  <h4 className="font-bold uppercase text-sm">Factories</h4>
                </div>
                <span className="font-bold text-gray-900">
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto pr-2">
                ...
                {breakdownParties.filter(p => p.party_type === 'factory').map(p => (
                  <div key={p.id} className="flex justify-between items-center py-1 border-b border-gray-50 last:border-0">
                    ...
                    <span className={`text-sm font-medium whitespace-nowrap ${Number(p.current_balance) > 0 ? 'text-red-600' : 'text-green-600'}`}>
                      ₹{Math.abs(Number(p.current_balance)).toLocaleString('en-IN')}
                    </span>
                  </div>
                ))}
            ...
            <div className="p-4">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2 text-pink-600">
                  ...
                  <h4 className="font-bold uppercase text-sm">Suppliers</h4>
                </div>
                <span className="font-bold text-gray-900">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <Link href={`/statements/${payment.party_id}`} className="font-semibold text-lg hover:text-blue-600 hover:underline">
                      {payment.party_name}
                    </Link>
                    <span className="text-[10px] px-2 py-0.5 bg-gray-100 rounded-full uppercase tracking-wider text-gray-600 font-medium">
                      {payment.party_type}
                    </span>
                  </div>
