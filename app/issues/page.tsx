      ...
      <div className="flex justify-between items-center mb-6 gap-2">
        <div className="flex items-center gap-2 sm:gap-4 min-w-0">
          ...
          <h2 className="text-lg sm:text-2xl font-bold flex items-center gap-2 truncate">
            <Factory className="w-5 h-5 sm:w-6 sm:h-6 flex-shrink-0" />
            <span className="truncate">Issues History</span>
          </h2>
        </div>
        <Link
          href="/issues/add"
        ...
        >
          ...
          <span className="hidden sm:inline">Issue Cloth</span>
      ...
      {loading ? (
        ...
        <div className="grid gap-3 sm:gap-4">
          {filteredIssues.map(issue => (
            <div key={issue.id} className="relative group bg-white rounded-xl p-3 sm:p-4 border flex flex-row items-center justify-between gap-3 sm:gap-4 transition-all hover:border-gray-400 hover:shadow-md">
              <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                {issue.color_image_url ? (
                  <img src={issue.color_image_url} alt={issue.cloth_color} className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg object-cover flex-shrink-0 border" />
                ) : (
                ...
                <div className="min-w-0">
                  <h3 className="font-semibold text-base sm:text-lg truncate">{issue.factory_name}</h3>
                  <p className="text-xs sm:text-sm text-gray-600 truncate">
                    {issue.cloth_name} {issue.cloth_color && `(${issue.cloth_color})`}
                  </p>
                  <p className="text-[10px] sm:text-xs text-gray-500 mt-0.5 truncate">Expected: {issue.product_type}</p>
                </div>
              </div>
